  ...
  )
{
  ...
  /* SCR */
    ...
    DEBUG ((DEBUG_ERROR, " ////////////////////////// Buffer address 0x%x\n", Buffer));
  ...
  if (EFI_ERROR (Status)) {
    ...
    int i;
    for (i=0; i<8; i++) {
      DEBUG ((DEBUG_ERROR, "buffer 0x%x\n", Buffer[i]));
    }

  DEBUG ((DEBUG_ERROR, "Scr.SPEC = 0x%x\n", Scr.SD_SPEC));
  DEBUG ((DEBUG_ERROR, "Scr.SPEC4 = 0x%x\n", Scr.SD_SPEC4));
  DEBUG ((DEBUG_ERROR, "Scr.BuS_W = 0x%x\n", Scr.SD_BUS_WIDTHS));
  DEBUG ((DEBUG_ERROR, "Scr.SCR = 0x%x\n", Scr.SCR_STRUCTURE));
 ...
 // Width ok and SCR value well read thanks to DMA

   Status = MmcHost->SendCommand (MmcHost, MMC_CMD13, 0xaaaa0000);
   if (EFI_ERROR (Status)) {
     DEBUG ((DEBUG_ERROR, "%a (MMC_CMD13): Error and Status = %r\n", __func__, Status));
     return Status;
   }
  ...
  )
{
  ...
  //Status = MmcHost->ReceiveResponse (MmcHost, 0xFF, Response);
  if (EFI_ERROR (Status)) {
